...
def build_dejavu_response(stimulus: str, conversation: ConversationState,
    ...
    if dejavu_count == 1:
        ...
        candidates = [t for t in DEJAVU_STAGE1_TEMPLATES if t not in used]
        if not candidates:
            candidates = DEJAVU_STAGE1_TEMPLATES
        ...
        used = set(conversation.response_history)
        candidates = [t for t in DEJAVU_STAGE3_TEMPLATES if t not in used]
        if not candidates:
            candidates = DEJAVU_STAGE3_TEMPLATES
        return random.choice(candidates)


# ============================================================================
# TASTE TEMPLATES — preserved but inactive until TASTE is re-enabled
# ============================================================================
