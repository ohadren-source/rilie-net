...
class RILIE:
    ...
    def process(
        ...
        # ✅ READ MEANING — the substrate layer
        meaning_fp = read_meaning(original_question)
        ...
        # -----------------------------------------------------------------
        # DDD — Discourse Dictates Disclosure
        # -----------------------------------------------------------------
        disclosure = self.conversation.disclosure_level

        # TASTE is bypassed — skip straight to Kitchen for all turns
        # Kitchen (interpretation passes) will handle all disclosure levels
        ...
        # ✅ MEANING FINGERPRINT — read-only birth certificate
        raw["meaning_fingerprint"] = meaning_fp
