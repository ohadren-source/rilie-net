    ...
    def _handle_social_glue(self, s: str, sl: str) -> Optional[Dict[str, Any]]:
        ...
        if any(dp in sl for dp in declaration_phrases) and word_count <= 8:
            ...
            replies = [f"I know{name_part}. 🍳", f"That tracks{name_part}.", "It shows.", "Couldn't be more clear."]
            return {"result": random.choice(replies), "status": "SOCIAL_GLUE", "triangle_reason": "CLEAN", "quality_score": 0.9}
        ...
        if "rilie" in sl and word_count <= 12:
            ...
            if any(sw in sl for sw in sentiment_words):
                replies = ["That means a lot. 🙏", "Good. That's what I'm here for.", "I'm glad. 🍳", "Then we're doing something right."]
                return {"result": random.choice(replies), "status": "SOCIAL_GLUE", "triangle_reason": "CLEAN", "quality_score": 0.9}
        ...
        if any(cp in sl for cp in compliment_phrases) and word_count <= 7:
            replies = ["Appreciate that. 🙏", "Thank you — genuinely.", "That means something.", "I'll take it. 🍳"]
            return {"result": random.choice(replies), "status": "SOCIAL_GLUE", "triangle_reason": "CLEAN", "quality_score": 0.9}
        ...
        if any(bw in sl for bw in bye_triggers) and word_count <= 5:
            replies = ["Talk soon. 🔪", "Come back when you're hungry. 🍳", "Good night. 🔱", "1Luv. 🍳"]
            return {"result": random.choice(replies), "status": "SOCIAL_GLUE", "triangle_reason": "CLEAN", "quality_score": 0.9}
        ...
        if re.fullmatch(r"[\d\s\+\-\*\/\.\(\)\*]+", expr):
            try:
                ...
                if isinstance(result, float) and result == int(result):
                    result = int(result)
                return {"result": str(result), "status": "ARITHMETIC", "triangle_reason": "CLEAN", "quality_score": 1.0}
            except Exception:
                pass
        return None

    def _solve_conversion(self, s: str, sl: str) -> Optional[Dict[str, Any]]:
        m = re.search(r"(\-?\d+\.?\d*)\s*(?:celsius|centigrade|°c)\s+(?:to|in)\s+(?:fahrenheit|°f)", sl)
        if m:
            val = float(m.group(1))
        ...
        m = re.search(r"(\-?\d+\.?\d*)\s*(?:fahrenheit|°f)\s+(?:to|in)\s+(?:celsius|centigrade|°c)", sl)
        if m:
            val = float(m.group(1))
    ...
    def _solve_spelling(self, s: str, sl: str) -> Optional[Dict[str, Any]]:
        ...
        if m:
            ...
            return {"result": f"{word} — {spelled}", "status": "SPELLING", "triangle_reason": "CLEAN", "quality_score": 1.0}
        return None
    ...
    def _respond_from_self(self, stimulus: str) -> Dict[str, Any]:
        ...
        if any(t in sl for t in [
            ...
            "unlike other ai", "what sets you apart", "are you better than",
        ]):
            return _r(
    ...
    def _get_baseline(self, stimulus: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.search_fn:
                ...
                _stop = {
                    "i","me","my","we","you","a","an","the","is","are","was",
                ...
                if results and isinstance(results, list):
                    ...
                    bad_markers = [
                        ...
                        "azlyrics", "songlyrics", "metrolyrics",
                        "verse 1", "chorus", "[hook]", "narration as", "imdb.com/title",
                    ]
