...
from guvna.service import init_guvna, run_rilie_service  # Guvna wiring via service layer
...
wired_search_fn: Optional[SearchFn] = (
    brave_search_sync if HAS_BRAVE_SEARCH else None
)

# Initialize the Guvna singleton (Act 5) via the service layer
init_guvna(roux_seeds=roux_seeds, search_fn=wired_search_fn)


# ---------------------------------------------------------------------------
# Static / Client — serves rilie-client.html at root
# ---------------------------------------------------------------------------

@app.get("/")
def serve_client():
    """Serve the RILIE client at root."""
...
def run_rilie(req: RilieRequest) -> Dict[str, Any]:
    ...
    Response includes:
      - result: The composed response
      - status: COMPRESSED, GUESS, RESONANCE, NO_PULSE, etc.
      - harmony, syncopation, synchronicity: Harmonic scores
      - meaning_fingerprint: MeaningFingerprint object with:
        - pulse: 0.0-1.0 (alive or dead signal)
        - act: GET, GIVE, or SHOW (primary intent)
        - act2: optional secondary intent
        - object: irreducible subject (what's this about?)
        - weight: 0.0-1.0 (how much it matters to user)
        - gap: what's missing / implied
      
      ✅ meaning_fingerprint should be logged to PostgreSQL for adaptive learning
      ✅ Tone, quality_score, priorities_met, anti_beige_score also present
    """
    ...
    result = run_rilie_service(stimulus, max_pass=req.max_pass)
    return result
...
def generate_file(req: GenerateFileRequest) -> Dict[str, Any]:
    ...
    result = run_rilie_service(stimulus, max_pass=3)
...
async def pre_response(req: PreResponseRequest) -> PreResponseResponse:
    """
    Skeleton PRE-RESPONSE endpoint:

    - If question is clear and shallow: Brave-search Q, count results.
    - Else: future expansion to Roux grid + BANKS.
    """
