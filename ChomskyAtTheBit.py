...
CHOMSKYATBIT.py — GRAMMAR BRAIN 2.0
...
_NAME_INTRO_PATTERNS = [
    ...
    # FIX v2.2: corrected backslash escaping (was \\s+, now \s+) so regex fires in production
    # FIX v2.2: added "name is" and "name's" as real-world parlance patterns
    r"(?:my name is|my name's|name is|name's|i am called|i'm called|call me|they call me|introduce myself(?: as)?)\s+([A-Za-z][A-Za-z\s'\-]{1,30}?)(?:\s*[.,!?]|\.?\s*[Aa] man|\s+(?:sometimes|usually|often|now|please|ok|okay|actually|just|btw|lol|right|though|too|also)|\s*$)",
